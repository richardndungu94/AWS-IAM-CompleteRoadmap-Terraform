 ...
 ##Terraform AWS IAM Baseline
  ...
  This project provides a secure and scalable baseline for managing AWS Identity and Access Management (IAM) using Terraform. It leverages
  public Terraform modules to configure account-wide settings, create users, and manage group memberships in a data-driven way.

  The configuration is designed to be modular and environment-aware, supporting separate configurations for development and production
  environments.
  ...
  Before you begin, ensure you have the following:
   ...
   3. AWS Credentials: Your AWS credentials must be configured in a way that Terraform can use them (e.g., via environment variables
      AWS_ACCESS_KEY_ID and AWS_SECRET_ACCESS_KEY, or an ~/.aws/credentials file).
   4. PGP Key: A PGP key is required to encrypt the initial IAM user passwords. You can use a service like Keybase.io (https://keybase.io) to
      get one easily.
  ...
  The primary configuration happens in the .tfvars files inside the environments/ directory.
   ...
   1     users = {
   2       "new_user_key" = {
   3         name  = "new.user.name"
   4         group = "developers" # or "admins"
   5       }
   6     }

  Step 2: Initialize Terraform

  Navigate to the root of the iam-project/ directory in your terminal and run this command once. This will download the necessary providers
  and modules.
  ...
  To deploy the `development` environment:
   ...
   1 # See what changes Terraform will make
  ...
  To deploy the `production` environment:
   ...
   3 
   4 # Apply the changes
   5 terraform apply -var-file="environments/prod.tfvars"
